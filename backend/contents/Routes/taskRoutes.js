...
router.get("/admin/tasks/all", async (req, res) => {
  try {
    ...
    const ticketTasks = openTickets.map((ticket) => ({
      title: `Support: ${ticket.subject}`,
      customer: ticket.user?.name || ticket.user?.email || "Unknown",
      status: ticket.status === "Closed" ? "Completed" : ticket.status, // Map statuses appropriately
...
router.delete("/admin/tasks/delete", async (req, res) => {
  try {
    ...
    if (!title || !dueDate)
      return res.status(400).json({ error: "Missing title or dueDate" });
...
router.post("/admin/tasks/seed", async (req, res) => {
  try {
    ...
    if (count > 0)
      return res.status(400).json({ message: "Tasks already exist" });
